  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <div className={`flex border rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-primary ${
        error || (!isValid && phoneNumber.length > 0) ? 'border-red-500' : 'border-gray-300'
      }`}>
        {/* Country Code Selector */}
        <button
          type="button"
          onClick={() => setIsOpen(!isOpen)}
          className="flex items-center gap-1 px-3 py-2 bg-gray-50 border-r border-gray-300 hover:bg-gray-100 min-w-[100px]"
        >
        ...
        <div className="absolute left-0 top-full mt-1 w-72 bg-white border border-gray-200 rounded-lg shadow-lg z-50 max-h-64 overflow-hidden">
          ...
          <div className="max-h-48 overflow-y-auto">
            {filteredCountries.length === 0 ? (
              <div className="p-3 text-center text-gray-500 text-sm">No countries found</div>
            ) : (
              filteredCountries.map((country, idx) => (
                ...
                >
                  <span className="w-12 font-medium text-gray-700">{country.code}</span>
                  <span className="flex-1 text-sm text-gray-900">{country.country}</span>
                  {selectedCountry.iso === country.iso && selectedCountry.code === country.code && (
